...
export default function ExpenseForm() {
    ...
    useEffect(() => {
        if (state.editingId) {
            const editinExpense = state.expenses.filter(currentExpense => currentExpense.id === state.editingId)[0]
            setExpense(editinExpense)
            setPreviousAmount(editinExpense.amount)
        }
    }, [state.editingId])
    ...
    const handleSubmit = ((e: FormEvent<HTMLFormElement>) => {
        ...
        if (expense.amount - previousAmount > remainingBudget) {
            setError('No puedes gastar más del presupuesto')
        }
